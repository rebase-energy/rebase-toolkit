...
Commands:
  ./scripts/dev_setup_local.sh --print-command   Print the setup command without running it
...
if [[ -n "${REBASE_DEV_REPO_SCOPE:-}" ]]; then
  args+=(--repo-scope "${REBASE_DEV_REPO_SCOPE}")
fi
if [[ -n "${REBASE_DEV_REPO:-}" ]]; then
  args+=(--repo "${REBASE_DEV_REPO}")
fi
if [[ -n "${REBASE_DEV_PROJECT:-}" ]]; then
  args+=(--project "${REBASE_DEV_PROJECT}")
fi
